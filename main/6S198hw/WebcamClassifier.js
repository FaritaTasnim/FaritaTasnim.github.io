...
//Farita's variables
var globLogConf = [0,0,0];
var globSafeConfFour = [0,0,0];
...
function onSpecialButtonClick() {
  ...
  window.alert('Number of the top ' + TOPK +  ' closest matches in each class: ' + globNCounts + '\n' 
  + 'Confidence for which the image matches each class: ' + globConf + '\n'
  + 'Log confidence for which the image matches each class: ' + globLogConf + '\n'
  + 'Safe confidence for which the image matches each class: ' + globSafeConfFour);
}
...
class WebcamClassifier {
  ...
  animate() {
    ...
    if (this.isDown) {
      this.math.scope(() => {
        if (this.currentClass.index == 0) {
          if (this.current.imagesCount < 20) {
            this.saveTrainingLogits(this.current.index);
            this.animate_helper();
          }
          else{
            window.alert('No more images can be input.');
            this.currentClass.index = 3;
            this.current.imagesCount=0;
          }
        }
        if (this.currentClass.index == 1) {
          if (this.current.imagesCount < 100) {
            this.saveTrainingLogits(this.current.index);
            this.animate_helper();
          }
          else{
        ...
        if (this.currentClass.index == 2) {
          if (this.current.imagesCount < 500) {
            this.saveTrainingLogits(this.current.index);
            this.animate_helper();
          }
          else{
      ...
      const computeConfidences = () => {
        ...
        //Computing a different "confidence" I'm calling "logConfidences" which computes ln(x_i)/sumln(x_n) for each class
        let logs = [];
        for (let index = 0; index < nCounts.length; index += 1) {
          if (nCounts[index] == 0) {
            logs[index] = 0;
          }
          else{
            logs[index] = Math.log(nCounts[index]);
          }
        }
        ...
        let logConfidences = [];
        for (let i = 0; i < logs.length; i+= 1){
          logConfidences[i] = logs[i]/logsum;
        }

        //Computing a different confidence I'm calling "safeConfidenceFour" which equally divides probability between any classes with at least 4 topK images
        let numFour = 0;
        let indexes = [];
        for (let index = 0; index < nCounts.length; index += 1) {
          if (nCounts[index] >= 4) {
        ...
        let safeConfidenceFour = [];
        for (let i = 0; i < nCounts.length; i+= 1){
          if (indexes.includes(i)){
            safeConfidenceFour[i] = 1.0/numFour;
          }
          else {
            safeConfidenceFour[i] = 0;
          }
        }
        ...
        globNCounts = nCounts;
        globLogConf = logConfidences;
        globSafeConfFour = safeConfidenceFour;
        ...
        console.log('Log confidence for which the image matches each class: ' + logConfidences);
        console.log('Safe confidence for which the image matches each class: ' + safeConfidenceFour);

        GLOBALS.learningSection.setConfidences(logConfidences);
